...
function toggle() {
  ...
  localStorage.setItem('mduiThemeLayoutDark', isDark);
  updateToggleButton(isDark);
}

function updateToggleButton(isDark) {
  const icon = document.getElementById('dark_toggle_icon');
  icon.textContent = isDark ? '' : '';
  document.getElementById('dark_toggle_btn').setAttribute('mdui-tooltip', 
    `{content: '切换为${isDark ? '亮色/暗色' : '亮色/暗色'}模式'}`);
}

// Initialize theme from localStorage
const savedTheme = localStorage.getItem('mduiThemeLayoutDark');
if (savedTheme === 'true') {
  document.body.classList.add('mdui-theme-layout-dark');
}
updateToggleButton(savedTheme === 'true');
